"""
AKOrN Classification Training Script

ORIGEN:
- Código base: AKOrN (ICLR 2025) - https://arxiv.org/abs/2410.13821
- Repositorio original: [URL del repo original de AKOrN]

MODIFICACIONES (ACRCris - Noviembre 2025):
1. Función _resolve_device() para soporte MPS (Apple Silicon) - Líneas 26-47
2. Dataset MNIST añadido - Líneas 219-235
3. Device como parámetro en funciones (no hardcoded "cuda") - Varias líneas
4. Scripts de setup para MNIST: setup_mnist.py, train_mnist.sh, etc.

Para más detalles ver: ORIGEN_Y_MODIFICACIONES.md
"""
...
# MODIFICACIÓN PROPIA: Soporte para MPS (Apple Silicon)
def _resolve_device(device_arg: str) -> torch.device:
    """Resolve runtime device with optional override."""
    ...
    if normalized != "auto":
        ...
        if requested.type == "mps" and not _mps_available():
...
def evaluate_model(net, testloader, criterion, device, eps=0.0, attack_method="fgsm"):
...
def train_epoch(net, epoch, ema, trainloader, optimizer, criterion, device):
    """Train for one epoch"""
    net.train()
    running_loss = 0.0
    n = 0
...
def adversarial_evaluation(writer, epoch, net, testloader, criterion, eps=8/255, eval_pgd=False, prefix="model/", device=None):
    """Comprehensive adversarial evaluation"""
    if device is None:
        raise ValueError("Device must be provided for adversarial_evaluation")
    # Clean accuracy
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Runtime device (auto-detect by default)",
    )
