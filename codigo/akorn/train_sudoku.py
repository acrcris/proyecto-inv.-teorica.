...
if __name__ == "__main__":
    ...
    # Resume options
    parser.add_argument("--resume", type=str, default=None, help="path to checkpoint_*.pth to resume training")
    parser.add_argument("--resume_ema", type=str, default=None, help="optional path to ema_*.pth to resume EMA state")
    ...
    # Enable CUDA-only speed features when available
    if device.type == "cuda":
    ...
    # Optional CPU core pinning only when supported and requested
    if hasattr(os, "sched_setaffinity") and args.limit_cores_used:
    ...
    # Optionally resume from checkpoint and EMA weights
    start_epoch = 0
    if args.resume is not None and os.path.isfile(args.resume):
        print(f"Resuming from checkpoint: {args.resume}")
        ckpt = torch.load(args.resume, map_location=device)
        net.load_state_dict(ckpt["model_state_dict"])  # model weights
        try:
            optimizer.load_state_dict(ckpt["optimizer_state_dict"])  # optimizer state
        except Exception as e:
            print(f"Warning: could not load optimizer state: {e}")
        # Continue after the saved epoch
        if "epoch" in ckpt:
            start_epoch = int(ckpt["epoch"]) + 1
            print(f"Starting at epoch {start_epoch}")
    if args.resume_ema is not None and os.path.isfile(args.resume_ema):
        print(f"Resuming EMA from: {args.resume_ema}")
        ema_ckpt = torch.load(args.resume_ema, map_location="cpu")
        # save_model stores { 'epoch': ..., 'model_state_dict': ... }
        key = "model_state_dict" if "model_state_dict" in ema_ckpt else None
        if key is None:
            print("Warning: EMA file missing 'model_state_dict'; skipping EMA resume.")
        else:
            ema.load_state_dict(ema_ckpt[key])
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for X, Y, is_input in tqdm.tqdm(trainloader):
            ...
            if args.speed_test and device.type == "cuda":
